...
/// (the dynamic SQL engine's "seed only when provably constant, taint otherwise" rule
/// applies here identically). Computed straight from the SAME fold state
/// the dynamic SQL engine already builds while walking a procedure body for its own
/// EXEC sites - an OUTPUT parameter is just an ordinary local variable inside that body, so
/// whatever the dynamic SQL engine proved it holds at the end of the body IS the value
