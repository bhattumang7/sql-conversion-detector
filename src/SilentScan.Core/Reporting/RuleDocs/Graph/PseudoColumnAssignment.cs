...
internal static class PseudoColumnAssignment
{
    ...
    public static RuleDocContent Content { get; } = new(
        WhyItMatters: """
            ...
            Oracle-confirmed on both statement shapes: an `INSERT` naming `$node_id`/`$edge_id` in
            ...
            computed column..."). This is decidable purely from the statement's own column
            references - no catalog lookup is needed, since the restriction holds for every graph
            node/edge table's `$node_id`/`$edge_id`, unconditionally.
            """,
        HowToFixIt: "Remove $node_id/$edge_id from the INSERT column list or UPDATE SET clause - the engine assigns and maintains these values itself.",
        Examples:
        [
            new RuleDocExample(
