...
- [ ] **Per-rule pages: fill the remaining ~104/234 rules.** Shipped:
      ...
      (`RulesDocGeneratorTests`) byte-compares against `docs/`. 130/234 rules
      ...
      control-flow-risk, statement-shape, database-configuration) — a rule
      with no entry still renders (short rationale only, humanized title,
      no fabricated fix/example section), just thinner. Remaining backlog:
      index-design (~20), formatting/dead-code/duplication/deprecated-
      syntax/code-metrics (~50, lower value - mostly self-evident from
      their name), dynamic-sql, lineage, and the rest of catalog
