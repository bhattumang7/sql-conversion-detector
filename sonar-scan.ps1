...
try {
    ...
    #  - docs/rules/** is excluded from COPY-PASTE DETECTION ONLY (cpd), not from
    #    analysis: those pages are generated by the CLI's rules-doc verb from
    #    RuleCatalog, one per rule, so they share a page skeleton by construction.
    #    Measured 2026-08-19: 19,202 of the project's 19,569 duplicated lines (98%)
    #    sat in docs/rules at 78.8% density across 234 files, against 288 lines
    #    (0.5%) in src and zero in tests - the metric was reporting the generator's
    #    template, not anything a person wrote or could refactor. They stay scanned
    #    for every other rule, so a real defect in a generated page still surfaces.
    $beginArgs = @(
        "/k:$ProjectKey"
        "/n:SilentScan"
